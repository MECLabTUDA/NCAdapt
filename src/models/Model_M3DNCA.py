...
from src.models.Model_BasicNCA3D import *
import random
import math
...
class M3DNCA(nn.Module):
    ...
    """
    def __init__(self, channel_n, fire_rate, device, steps=64, hidden_size=128, input_channels=1, output_channels=1, scale_factor=4, levels=2, kernel_size=7, **kwargs):
        r"""Init function
            #Args:
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor = None, batch_duplication=1, calc_flops=False):
        # -- NOTE: Used for eval to get some metrics -- #
        if not self.training and y is None:
            if calc_flops:  #NOTE: Set to true when ptflops is used, so the dimensions are set correctly
                x = torch.moveaxis(x, 0, -1)
            y = torch.zeros_like(x) # --> NOTE: This should not be used when we are interested in Dice score, only when we want to generate flops etc. since external modules only provide an x not a y and the code will throw an error otherwise!
    ...
    def __init__(self, *args, **kwargs):
        r"""Init function
            #Args:
                ...
                input_channels: number of input channels
        """
        super(M3DNCA_CL, self).__init__(*args, **kwargs)

    def _freeze_convs(self):
        for name, param in self.model.named_parameters():
            if 'p0' not in name:
                param.requires_grad = False # Freeze this layer

class M3DNCA_CL_new_module(M3DNCA):
    def __init__(self, *args, **kwargs):
        r"""Init function
            #Args:
                channel_n: number of channels per cell
                ...
                input_channels: number of input channels
        """
        super(M3DNCA_CL_new_module, self).__init__(*args, **kwargs)
        self.model = nn.ModuleList()
        for i in range(self.levels):
            ks = self.kernel_size if i == 0 else 3
            self.model.append(BasicNCA3D_CL_simple(channel_n=self.channel_n, fire_rate=self.fire_rate, device=self.device, hidden_size=self.hidden_size, input_channels=self.input_channels, kernel_size=ks))

    def _freeze_modules(self):
        # Set weights of self.adjust_0 and self.adjust_1 to 1 and freeze them
        for model_ in self.model:
            model_.adjust_0.weight = torch.nn.Parameter(torch.ones_like(model_.adjust_0.weight))
            model_.adjust_1.weight = torch.nn.Parameter(torch.ones_like(model_.adjust_1.weight))
        for name, param in self.model.named_parameters():
            if 'adjust' in name:
                param.requires_grad = False # Freeze this layer

    def _un_freeze_modules(self):
        # Set weights of self.adjust_0 and self.adjust_1 to 1 and freeze them
        for name, param in self.model.named_parameters():
            if 'adjust' in name:
                param.requires_grad = True # Un-Freeze this layer

class M3DNCA_CL_new_module_dict(M3DNCA):
    def __init__(self, *args, **kwargs):
        r"""Init function
            #Args:
                channel_n: number of channels per cell
                ...
                input_channels: number of input channels
        """
        super(M3DNCA_CL_new_module_dict, self).__init__(*args, **kwargs)
        self.model = nn.ModuleList()
        for i in range(self.levels):
            ks = self.kernel_size if i == 0 else 3
            self.model.append(BasicNCA3D_CL_two_adjusts(channel_n=self.channel_n, fire_rate=self.fire_rate, device=self.device, hidden_size=self.hidden_size, input_channels=self.input_channels, kernel_size=ks, nr_modules=kwargs['nr_modules']))
            # self.model.append(BasicNCA3D_CL(channel_n=self.channel_n, fire_rate=self.fire_rate, device=self.device, hidden_size=self.hidden_size, input_channels=self.input_channels, kernel_size=ks, nr_modules=kwargs['nr_modules']))

    def _freeze_modules_1(self, id):
        # Set weights of adjust_0 and adjust_1 to 1 and freeze them
        for model_ in self.model:
            model_.adjusts[str(id)]['adjust_0'].weight = torch.nn.Parameter(torch.ones_like(model_.adjusts[str(id)]['adjust_0'].weight))
            model_.adjusts[str(id)]['adjust_1'].weight = torch.nn.Parameter(torch.ones_like(model_.adjusts[str(id)]['adjust_1'].weight))
        for name, param in self.model.named_parameters():
            if str(id)+'.adjust_' in name:
                param.requires_grad = False # Freeze this layer

    def _freeze_modules(self, id, clone_state=None):
        if clone_state is not None:
            for model_ in self.model:
                model_.adjusts[str(id)]['adjust_0'].weight = torch.nn.Parameter(model_.adjusts[str(clone_state)]['adjust_0'].weight.clone())
                model_.adjusts[str(id)]['adjust_1'].weight = torch.nn.Parameter(model_.adjusts[str(clone_state)]['adjust_1'].weight.clone())
        # Set weights of adjust_0 and adjust_1 to 1 and freeze them
        for name, param in self.model.named_parameters():
            if str(id)+'.adjust_' in name:
                param.requires_grad = False # Freeze this layer

    def _un_freeze_modules(self, id):
        # Set weights of self.adjust_0 and self.adjust_1 to 1 and freeze them
        for name, param in self.model.named_parameters():
            if str(id)+'.adjust' in name:
                param.requires_grad = True # Un-Freeze this layer

    def _use_module(self, module_id):
        for model_ in self.model:
            model_.use_module = str(module_id)

class M3DNCA_CL_new_module_dict_new(M3DNCA_CL_new_module_dict):
    def __init__(self, *args, **kwargs):
        r"""Init function
            #Args:
                channel_n: number of channels per cell
                ...
                input_channels: number of input channels
        """
        super(M3DNCA_CL_new_module_dict_new, self).__init__(*args, **kwargs)
    ...
    def _un_freeze_modules(self, id):
        # Set weights of self.adjust_0 and self.adjust_1 to 1 and freeze them
        for name, param in self.model.named_parameters():
            if str(id)+'.adjust' not in name:
                param.requires_grad = True # Un-Freeze this layer

class M3DNCA_CL_new_module_dict_new_no_freeze_convs(M3DNCA_CL_new_module_dict_new):
    def __init__(self, *args, **kwargs):
        r"""Init function
            #Args:
                channel_n: number of channels per cell
                ...
                input_channels: number of input channels
        """
        super(M3DNCA_CL_new_module_dict_new_no_freeze_convs, self).__init__(*args, **kwargs)
    ...
    def _un_freeze_convs(self):
        for name, param in self.model.named_parameters():
            if 'p0' in name:
                param.requires_grad = True # Un-Freeze this layer

class M3DNCA_CL_new_module_dict_new_p0s(M3DNCA):
    def __init__(self, *args, **kwargs):
        r"""Init function
            #Args:
                channel_n: number of channels per cell
                ...
                input_channels: number of input channels
        """ 
        super(M3DNCA_CL_new_module_dict_new_p0s, self).__init__(*args, **kwargs)
        self.model = nn.ModuleList()
        for i in range(self.levels):
            ks = self.kernel_size if i == 0 else 3
            self.model.append(BasicNCA3D_CL_two_adjusts_and_conv_adjust(channel_n=self.channel_n, fire_rate=self.fire_rate, device=self.device, hidden_size=self.hidden_size, input_channels=self.input_channels, kernel_size=ks, nr_modules=kwargs['nr_modules']))
    ...
    def _freeze_modules(self, id, clone_state=None):
        if clone_state is not None:
            for model_ in self.model:
                model_.adjusts[str(id)]['adjust_0'].weight = torch.nn.Parameter(model_.adjusts[str(clone_state)]['adjust_0'].weight.clone())
                model_.adjusts[str(id)]['adjust_1'].weight = torch.nn.Parameter(model_.adjusts[str(clone_state)]['adjust_1'].weight.clone())
                model_.p0s[str(id)].weight = torch.nn.Parameter(model_.p0s[str(clone_state)].weight.clone())
        # Set weights of adjust_0 and adjust_1 to 1 and freeze them
        for name, param in self.model.named_parameters():
            if str(id)+'.adjust_' in name or 'p0s.'+str(id) in name:
                param.requires_grad = False # Freeze this layer

    def _set_modules_1(self, id):
        # Set weights of adjust_0 and adjust_1 to 1 and freeze them
        for model_ in self.model:
            model_.adjusts[str(id)]['adjust_0'].weight = torch.nn.Parameter(torch.ones_like(model_.adjusts[str(id)]['adjust_0'].weight))
            model_.adjusts[str(id)]['adjust_1'].weight = torch.nn.Parameter(torch.ones_like(model_.adjusts[str(id)]['adjust_1'].weight))
    
    def _copyp0(self, id, clone_state):
        for model_ in self.model:
            model_.p0s[str(id)].weight = torch.nn.Parameter(model_.p0s[str(clone_state)].weight.clone())

    def _freeze_model_full(self):
        for _, param in self.model.named_parameters():
            param.requires_grad = False # Freeze this layer

    def _un_freeze_model_full(self):
        for _, param in self.model.named_parameters():
            param.requires_grad = True # Freeze this layer

    def _un_freeze_modules(self, id):
        # Set weights of self.adjust_0 and self.adjust_1 to 1 and freeze them
        for name, param in self.model.named_parameters():
            if str(id)+'.adjust' in name or 'p0s.'+str(id) in name:
                param.requires_grad = True # Un-Freeze this layer
...
class M3DNCA_CL_ablations(M3DNCA_CL):
    def __init__(self, *args, **kwargs):
        r"""Init function
            #Args:
                channel_n: number of channels per cell
                ...
                input_channels: number of input channels
        """
        super(M3DNCA_CL_ablations, self).__init__(*args, **kwargs)

    def _freeze_nca_0(self):
        # Set weights of adjust_0 and adjust_1 to 1 and freeze them
        for name, param in self.model.named_parameters():
            if '0.fc' in name or '0.p0' in name or '0.bn' in name:
                param.requires_grad = False # Freeze this layer

    def _freeze_nca_1(self):
        # Set weights of adjust_0 and adjust_1 to 1 and freeze them
        for name, param in self.model.named_parameters():
            if '1.f' in name or '1.p' in name or '1.b' in name:
                param.requires_grad = False # Freeze this layer

    def _freeze_no_convs(self):
        for name, param in self.model.named_parameters():
            if 'p0' not in name:
                param.requires_grad = False # Freeze this layer