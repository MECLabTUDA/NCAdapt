...
def store_config_args(func):
    ...
    function as a dictionary in 'self.model_config_'. This is used to assist
...
class LoadableModel(nn.Module):
    ...
    provided to __init__ are automatically saved into the object (in self.model_config_)
    ...
    def __init__(self, *args, **kwargs):
        if not hasattr(self, 'model_config_'):
    ...
    def save(self, path):
        ...
        for key in grid_buffers:
            sd.pop(key)
        torch.save({'model_config_': self.model_config_, 'model_state': sd}, path)

    @classmethod
    def load(cls, path, device='cuda:0', model='MedNCA'):
        """
        Load a python model configuration and weights.
        """
        checkpoint = torch.load(path, map_location=torch.device(device))
        checkpoint['model_config_']['device'] = device
        model = cls(**checkpoint['model_config_'])
        model.load_state_dict(checkpoint['model_state'], strict=False)
        # Set the device for the entire model
        model.device=device
        return model